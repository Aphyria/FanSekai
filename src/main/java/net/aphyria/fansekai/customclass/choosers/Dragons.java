...
public class Dragons extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if (interactionHand == InteractionHand.MAIN_HAND){
            player.getCapability(PlayerClansProvider.PLAYER_CLAN).ifPresent(clan -> {
                if (level.isClientSide && !clan.isInClan())player.sendSystemMessage(Component.literal("You are now part of the Jade Dragons!").withStyle(ChatFormatting.DARK_AQUA));
                if (level.isClientSide && clan.isInClan()){
                    if (clan.getClan() == 1)player.sendSystemMessage(Component.literal("You are already part of the Jade Dragons.").withStyle(ChatFormatting.DARK_AQUA, ChatFormatting.BOLD));
                    else {
                ...
                if (!clan.isInClan())clan.setClan(1);
            });
        }
