...
public class Warriors extends Item {
    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand interactionHand) {
        if (interactionHand == InteractionHand.MAIN_HAND){
            player.getCapability(PlayerClansProvider.PLAYER_CLAN).ifPresent(clan -> {
                if (level.isClientSide && !clan.isInClan())player.sendSystemMessage(Component.literal("You are now part of the Warriors Guild!").withStyle(ChatFormatting.YELLOW));
                if (level.isClientSide && clan.isInClan()){
                    if (clan.getClan() == 2)player.sendSystemMessage(Component.literal("You are already part of the Warriors Guild.").withStyle(ChatFormatting.YELLOW, ChatFormatting.BOLD));
                    else {
                ...
                if (!clan.isInClan())clan.setClan(2);
            });
        }
